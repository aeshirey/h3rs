 ...
 */
pub fn degsToRads(degrees:f64) -> f64{ degrees * M_PI_180 }
 ...
 */
pub fn radsToDegs(radians:f64) -> f64{ radians * M_180_PI }
 ...
 */
fn constrainLng(mut lng:f64) -> f64 {
    while (lng > M_PI) {
        lng = lng - (2 * M_PI);
    }
    while (lng < -M_PI) {
...
fn _geoAzimuthRads(p1:&Self, p2:&Self) -> f64 {
    f64::atan2(p2.lat.cos() * (p2.lon - p1.lon).sin(),
                 p1.lat.cos() * p2.lat.sin() -
                     p1.lat.sin() * p2.lat.cos() * (p2.lon - p1.lon).cos())
}
...
fn triangleArea(a:&Self, b:&Self, c:&Self) -> f64{
    Self::triangleEdgeLengthsToArea(Self::pointDistRads(a, b),
