...
const H3_SET_RESERVED_BITS(h3, v)            \
...
impl H3Index {
    fn H3_GET_RESOLUTION(&self) -> i32 {
        let H3Index(h3) = self;
        (h3 & H3_RES_MASK) >> H3_RES_OFFSET
    }
 ...
 */
fn h3GetResolution(&self) { H3_GET_RESOLUTION(self) }
 ...
 */
fn h3GetBaseCell(&self) -> u64 { H3_GET_BASE_CELL(self) }
...
fn h3IsValid(&self) -> bool {
    if H3_GET_HIGH_BIT(self) != 0  { return  false ; }

    if (H3_GET_MODE(self) != H3_HEXAGON_MODE) { return false; }

    if (H3_GET_RESERVED_BITS(self) != 0) { return false; }

    let baseCell = H3_GET_BASE_CELL(self);
    if (baseCell < 0 || baseCell >= NUM_BASE_CELLS) {  // LCOV_EXCL_BR_LINE
        // Base cells less than zero can not be represented in an index
        return false;
    }

    let res = H3_GET_RESOLUTION(self);
    ...
    for (int r = 1; r <= res; r++) {
        let digit : Direction = H3_GET_INDEX_DIGIT(self, r);
    ...
    for r in (res+1..=MAX_H3_Res) {
        let digit : Direction = H3_GET_INDEX_DIGIT(h, r);
...
fn h3ToParent(H3Index h, int parentRes) -> Self {
    let childRes : u64 = H3_GET_RESOLUTION(h);
 ...
 */
int h3IsResClassIII(H3Index h) { return H3_GET_RESOLUTION(h) % 2; }
 ...
 */
int pentagonIndexCount() { return NUM_PENTAGONS; }
